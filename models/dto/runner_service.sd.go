...
func (req *EvolverService_Evolve_Request) NewRequest(method, url string) (*http.Request, error) {
	buffer := new(bytes.Buffer)
	err := json.NewEncoder(buffer).Encode(req)
	if err != nil {
		return nil, errors.Wrap(err, "failed on serialization")
...
func (req *EvolverService_Evolve_Request) ParseRequest(r *http.Request) error {
	err := json.NewDecoder(r.Body).Decode(req)
	if err != nil {
		return errors.Wrap(err, "failed on parsing the response body")
	}
...
func (req *EvolverService_Evolve_Request) Send(method, url string) (*EvolverService_Evolve_Response, error) {
	httpRequest, err := req.NewRequest(method, url)
	if err != nil {
		return nil, errors.Wrap(err, "failed on creating an object for request")
	}
	...
	res := EvolverService_Evolve_Response{}
	err = res.DeserializeResponse(httpResponse)
	if err != nil {
		return nil, errors.Wrap(err, "failed on parsing the response body")
	}
...
func (res *EvolverService_Evolve_Response) SerializeIntoResponseWriter(w http.ResponseWriter) error {
	err := json.NewEncoder(w).Encode(res)
	if err != nil {
		return errors.Wrap(err, "failed on serialization")
	}
...
func (s *EvolverService_Evolve_Response) DeserializeResponse(res *http.Response) error {
	err := json.NewDecoder(res.Body).Decode(s)
	if err != nil {
		return errors.Wrap(err, "failed on parsing the response body")
	}
