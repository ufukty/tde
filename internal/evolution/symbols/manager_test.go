		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for pkgid, symbols := range got {
				...
				for _, id := range symbols {
					ss = append(ss, id.Name)
				}
				fmt.Printf("%s:\n  [%s]\n", pkgid, strings.Join(ss, ", "))
			}
			...
			for pkgid, expectedPkgSymbols := range tc.expectedSymbols {
				if gotPkgSymbols, ok := got[pkgid]; ok {
					gotPkgSymbolsStrings := []string{}
					for _, id := range gotPkgSymbols {
						gotPkgSymbolsStrings = append(gotPkgSymbolsStrings, id.Name)
					}
